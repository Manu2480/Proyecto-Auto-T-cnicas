...
class NaveEspacialBacktracking {
  ...
  puedeMover(fila, col, energia, visitadas, agujerosNegros) {
    if (fila < 0 || fila >= this.filas || col < 0 || col >= this.columnas) console.log(`Retrocediendo desde: (${fila}, ${col})`); return false;
    const clave = `${fila},${col}`;
    if (visitadas.has(clave)) console.log(`Retrocediendo desde: (${fila}, ${col})`); return false;
    if (this.esAgujeroNegro(fila, col, agujerosNegros)) console.log(`Retrocediendo desde: (${fila}, ${col})`); return false;
  ...
  backtracking(fila, col, energia, visitadas, ruta, agujerosNegros, estrellasGigantes, agujerosGusano, celdasEnergia) {
    ...
    if (ruta.length > this.limiteProfundidad) {
      console.warn('Límite de profundidad alcanzado');
      console.log(`Retrocediendo desde: (${fila}, ${col})`); return false;
    }
    
    
    console.log(`Visitando celda: (${fila}, ${col}) con energía: ${energia}, ruta actual: ${ruta.length}`);
    ...
    if (this.esEstrellaGigante(fila, col, estrellasGigantes)) {
      ...
      if (res.agujeroEliminado) {
        console.log(`🌟 Estrella en (${fila}, ${col}) destruyó agujero negro en (${res.agujeroEliminado})`);
      } else {
        console.log(`🌟 Estrella en (${fila}, ${col}) no encontró agujero negro adyacente, continúa sin detenerse.`);
      }
    }
    ...
    if (visitadas.has(`${newFila},${newCol}`)) {
      console.warn(`Destino del gusano ya visitado: (${newFila}, ${newCol})`);
      console.log(`Retrocediendo desde: (${fila}, ${col})`); return false;
    }
